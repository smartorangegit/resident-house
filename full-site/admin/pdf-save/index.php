...
require_once($dt.'/bd.php');
	$result = $DB->prepare("SELECT floor, 	sec, 	type, id FROM `apartments` WHERE visible=1 ORDER BY id LIMIT 20");
...
function scriptOffset (url, offset, action) {
		...
		$.ajax({
			url: "/admin/pdf-save/pdf-save-application.php",
			type: "POST",
			data: {
			    "action":action
			  , "url":url
			...
			console.log(data)
				...
				if(data.sucsess < 1) {
					...
					setCookie();
					$('.bar').css('width','100%');
					$('.bar').html('<a href="'+data.title+'">Скачать</a>');
					$('#runScript').text('Еще');
